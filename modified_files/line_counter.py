...
def transMat_xy2uw(line_start,line_end):
    xy = Vector(line_start, Point(line_end.x,line_start.y))
    uw = Vector(line_start,line_end)

    cos_theta = float(xy.magnitude()/uw.magnitude())
    if xy.end.y>=uw.end.y:
        sin_theta = np.sqrt(1 - (cos_theta)**2)

    else:
        sin_theta = -1*np.sqrt(1 - (cos_theta)**2)
...
class LineCounter:
    def __init__(self, start: Point, end: Point,all_vehicle_class):
        """
        ...
        #classwise_count
        self.classwise_count = {key: [0,0] for key in all_vehicle_class}

        self.transMat = transMat_xy2uw(start,end)
        self.u1 = np.matmul(self.transMat, np.array([start.x,start.y]))[0]
        self.u2 = np.matmul(self.transMat, np.array([end.x,end.y]))[0]

    def update(self, detections: Detections,corridor_id,turning_prop):
        """
        ...
        for xyxy, confidence, class_id, tracker_id in detections:
            ...
            self.tracker_state[tracker_id] = tracker_state
            B = np.array([(x1+x2)/2, (y1+y2)/2])
            u_cm = np.matmul(self.transMat, B)[0]
            
            if tracker_state : #and (self.u1 <= u_cm <= self.u2)
                self.in_count += 1

                self.classwise_count[int(class_id)][0] += 1

                turning_prop.setdefault(tracker_id, []).append(corridor_id)


            elif tracker_state==False:
                self.out_count += 1

                self.classwise_count[int(class_id)][1] += 1

                turning_prop.setdefault(tracker_id, []).append(corridor_id)
            
    
    def get_data(self):
        all_counts = self.classwise_count.copy()
        self.classwise_count = {key: [0,0] for key in list(all_counts.keys())}
        all_counts['total'] = [self.in_count,self.out_count]
        return all_counts
